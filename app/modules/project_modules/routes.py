...
def list_modules(project_id: UUID, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
@router.delete("/modules/{module_id}/assign")
def unassign_user(module_id: UUID, user_id: UUID, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return ProjectModuleService().unassign_user(db, module_id, user_id, current_user)


@router.get("/modules/{module_id}/assignments")
def list_assignments(module_id: UUID, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
